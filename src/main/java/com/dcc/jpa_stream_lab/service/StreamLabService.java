...
public class StreamLabService {
    ...
    public List<User> RProblemFive()
    {
        ...
        Calendar calendar1 = new GregorianCalendar(2016,1,1);
        Date date1 = calendar1.getTime();
        Calendar calendar2 = new GregorianCalendar(2018,1,1);
        Date date2 = calendar2.getTime();

        return users.findAll().stream().filter(p -> p.getRegistrationDate().after(date1)).filter(p -> p.getRegistrationDate().before(date2)).toList();
    }
    ...
    public List<Product> RProblemSix()
    {
        ...
        User aftonemail = users.findAll().stream().filter(u -> u.getEmail().equals("afton@gmail.com")).findFirst().orElse(null);
        List<ShoppingcartItem> aftonCart = shoppingcartitems.findAll().stream().filter(s -> s.getUser().equals(aftonemail)).toList();
        List<Product> aftonProducts = aftonCart.stream().map(a -> a.getProduct()).toList();
    ...
    public long RProblemSeven()
    {
        ...
        User odaemail = users.findAll().stream().filter(u -> u.getEmail().equals("oda@gmail.com")).findFirst().orElse(null);
        List<ShoppingcartItem> odaCart = shoppingcartitems.findAll().stream().filter(s -> s.getUser().equals(odaemail)).toList();
        List<Product> odaProduct= odaCart.stream().map(o -> o.getProduct()).toList();
        List<Integer> odaSum = odaProduct.stream().map(o -> o.getPrice()).toList();
        List<Integer> list = odaSum;
        int sum = 0;
